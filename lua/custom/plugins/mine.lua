    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('FileType', {
        ...
        callback = function(_)
          ...
          vim.keymap.set('n', '<leader>gtf',
            '<cmd>:GoTestFunc<CR>', { desc = 'Run [G]o [T]est [F]unction' }
          )
          vim.keymap.set('n', '<leader>gtl',
            '<cmd>:GoTestFile<CR>', { desc = 'Run [G]o [T]est fi[L]e' }
          )
          vim.keymap.set('n', '<leader>gi',
            '<cmd>:GoImports<CR>', { desc = 'Run [G]o [i]mports' }
          )
          vim.keymap.set('n','<leader>dg','', { desc='Debug [G]o' })
          vim.keymap.set( 'n','<leader>dgt',
            function() require('dap-go').debug_test() end, { desc='[D]ebug [G]o [T]est'}
          )
          vim.keymap.set( 'n','<leader>dgl',
            function() require('dap-go').debug_last_test() end, { desc='[D]ebug [G]o [L]ast test'}
          )
        end,
      })
      ...
      vim.keymap.set('n', '<leader>cpc',
        '<cmd>:CopilotChat<CR>', { desc = '[C]o[P]ilot[C]hat' }
      )
      vim.keymap.set('n', '<leader>cpm',
        '<cmd>:CopilotChatCommit<CR>', { desc = '[C]o[P]ilotChat co[M]mit' })
      vim.keymap.set('n', '<leader>cfr',
        '<cmd>let @*=expand("%")<CR>', { desc = '[C]opy [F]ile [R]elative path' }
      )
      vim.keymap.set('n', '<leader>cfp',
        '<cmd>let @*=expand("%:p")<CR>', { desc = '[C]opy [F]ile absolute [P]ath' })
      vim.keymap.set('n', '<leader>cfn',
        '<cmd>let @*=expand("%:t")<CR>', { desc = '[C]opy [F]ile [N]ame' }
      )
      vim.keymap.set('n', '<leader>cfvi .gvi .',
        '<cmd>.GBrowse!<CR>', { desc = '[C]opy [F]ile:line [G]itHub URL' }
      )
      ...
      --       '<cmd>:MavenTest<CR>', { desc = 'Run [M]aven [T]est function/class' }
      --     )
      --   end,
      -- })
      -- vim.keymap.set('n', '<leader>mc',
      --   '<cmd>:MavenClean<CR>', { desc = 'Run [M]aven [C]lean' }
      -- )
      -- vim.keymap.set('n', '<leader>mva',
      --   '<cmd>:MavenVerifyAll<CR>', { desc = 'Run [M]aven [V]erify all' }
      -- )
      -- vim.keymap.set('n', '<leader>mta',
      --   '<cmd>:MavenTestAll<CR>', { desc = 'Run [M]aven [T]test all' }
      -- )
      ...
      vim.keymap.set('n','<leader>dv',
        function() require('dapui').toggle() end, { desc='[D]ebug Toggle [V]iew'}
      )
      vim.keymap.set( 'n','<leader>dc',
        function() require('dap').continue() end, { desc='[D]ebug [C]ontinue/Start'}
      )
      vim.keymap.set( 'n','<leader>di',
        function() require('dap').step_into() end, { desc='[D]ebug Step [I]nto'}
      )
      vim.keymap.set( 'n','<leader>do',
        function() require('dap').step_over() end, { desc='[D]ebug Step [O]ver'}
      )
      vim.keymap.set( 'n','<leader>du',
        function() require('dap').step_out() end, { desc='[D]ebug Step o[U]t'}
      )
      vim.keymap.set( 'n','<leader>dt',
        function() require('dap').close() end, { desc='[D]ebug [T]erminate/Close'}
      )
